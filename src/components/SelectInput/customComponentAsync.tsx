import { Check, ChevronDown, Loader2Icon, LockIcon, X } from "lucide-react";
...
export const controlStyles = {
  ...
  multiSmall: "!min-h-8",
  focusOption: "bg-common-surfaceOverlay",
};
...
>({
  ...
  return (
    ...
    >
      ...
      <p className="uppercase">{props.children}</p>
    </components.MultiValue>
  );
};
  ...
  return (
    ...
    >
      {props.isSelected ? (
        <span
          id="icon-checked"
          className="inline-flex h-4 w-4 items-center justify-center rounded-sm border border-primary-300 bg-primary-300 text-neutral-dark-100"
        >
        ...
        <input
          ...
          onChange={() => null}
          className="h-4 w-4 shrink-0 rounded-sm border focus-visible:outline-none disabled:cursor-not-allowed disabled:opacity-50"
        />
      )}
