...
async function main() {
  ...
  if (name) {
    ...
    replaceInFile(path.resolve("README.md"), [new RegExp(oldName, "g"), name], dry);
  }

  // index.html <title>
  if (title) {
    const indexHtml = path.resolve("index.html");
    replaceInFile(indexHtml, [/\<title\>[^<]*\<\/title\>/, `<title>${title}</title>`], dry);
    // README intro line ("Pulsar is a modern ...")
    replaceInFile(path.resolve("README.md"), [/^Pulsar\b/gm, title], dry);
  }
